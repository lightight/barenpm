...
uwsServer.ws('/*', {
    ...
    open: (ws) => {
        ...
        ws.send(continuePacketMaker(0, 127), true, true);
    },
        ...
        try {
            if (wispFrame.type === CONNECT_TYPE.CONNECT) {
                ...
                if (connectFrame.streamType === STREAM_TYPE.TCP) {
                    ...
                    client.on("connect", () => {
                        ws.send(continuePacketMaker(wispFrame_streamID, 127), true);
                    });
                    ...
                    (async () => {
                        ...
                        client.on("connect", () => {
                            ws.send(continuePacketMaker(wispFrame.streamID, 127), true);
                        });

                        client.on("message", (data, rinfo) => {
                            ws.send(dataPacketMaker(wispFrame.streamID, data), true);
                        });

                        client.on("error", (error) => {
                            if (ws.readyState === ws.OPEN) {
                                try {
                                    ws.send(closePacketMaker(wispFrame.streamID, 0x3), true);
